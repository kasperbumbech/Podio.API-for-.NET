...
namespace Podio.API.Services
{
    public class OrganisationService
    {
        ...
        public User GetOrganizationMember(int orgId,int userId) {
            ...
            return PodioRestHelper.Request<User>(Constants.PODIOAPI_BASEURL + "/org/" + orgId + "/member/" + userId, _client.AuthInfo.access_token).Data;
        }
        ...
        {
            // im not entirely sure that this is working as intended since this feature requires an upgraded account
            return PodioRestHelper.Request<List<User>>(Constants.PODIOAPI_BASEURL + "/org/" + orgId + "/member/", _client.AuthInfo.access_token).Data;
        }
        ...
        public Space GetSpaceByUrl(int orgId,string url) {
            return PodioRestHelper.Request<Space>(Constants.PODIOAPI_BASEURL + "/org/" + orgId + "/space/", _client.AuthInfo.access_token, new Dictionary<string, string> { { "url", url } }).Data;
        }
        ...
        {
            return PodioRestHelper.Request<List<Space>>(Constants.PODIOAPI_BASEURL + "/org/" + orgId + "/space/", _client.AuthInfo.access_token).Data;
        }
